...
\begin{definition}[Producto interno]
  Llmamamos producto interno en $V$ a una función $\ip{x,y}: V\times V\to \R$ que cumple \begin{enumerate}
...
\end{proposition}
Para demostrar que cumple la desigualdad triangular usaremos el siguiente teorema.
...
\end{theorem}
\begin{proof}
  A mí me gusta la demostración usando la descomposición en vectores ortogonales. La profesora ha dado otra.
\end{proof}
...
\begin{proof}
  ...
  Se deja como tortura para el lector verificar que el enunciado converso es cierto.
\end{proof}
...
\end{definition}
\begin{example}
  Sea $V=\R^2$, ilustramos las bolas abiertas con centro 0 y radio 1 para las normas $L_p$, $p=1,2,3,4,\infty$ en la figura \ref{fig:tema_1_bolas_lp}
...
Si $(x_n)\leadsto x$ e $(y_n)\leadsto y$, entonces 
\begin{itemize}
  \item $(\norm{x_k})\leadsto \norm{x}$ 
  \item para una sucesión real $(\lambda^k)\leadsto \lambda$, $(\lambda^k x^k+y^k)\leadsto \lambda x+y$
  \item $(\ip{x^k, y^k})\leadsto \ip{x,y}$ 
\end{itemize}
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{lemma}
  En $\R^n$ una sucesion converge, $(x^n)\leadsto x$, si y solo si sus componentes convergen, $(x^k_j)\leadsto x_j$ para $1\leq j\leq n$.
\end{lemma}
\begin{proof}
  ...
  \]
  Asume que $(x^k)\leadsto x$; sea $\epsilon>0$, existe $n_0\in \N$ tal que si $k \ge n_0$, $\norm{x^k-x} \le C_1\epsilon$ y por la cadena de desigualdades anterior esto implica que $(x_i^k)\leadsto x_i$.\\
  Asume que para todo $i\in \{1,\ldots,n\}$, $(x_i^k)\leadsto x_i$; sea $\epsilon >0$, existen $n_0^i \in \N$ tal que $|x_i^k-x_i|<\frac{\epsilon }{C_2}$ para $1 \leq i \leq n$; para $k \geq n_0$, donde $n_0=\max_{i} \{n_0^i\}$, tenemos que $\max_{j} \{|x_j^k-x_j\} \leq \frac{\epsilon }{C_2}$ y por la cadena de desigualdades anterior esto implica que $(x^k)\leadsto x$ 
\end{proof}
