...
\input{fig/discriminante_parabola.tex}
\begin{proof}
...
\input{fig/ley_paralelogramo.tex}
...
\end{lemma}
 ...
 \end{proof}
\begin{informal}
  Ya definimos función convexa en $\R$ pidiendo $f''\geq 0$. Resulta que esto es equivalente a pedir que la cuerda entre dos puntos cualesquiera de la gráfica quede por encima de la curva: si tomamos $x,y$ en el dominio y caminamos por el segmento $tx+(1-t)y$, el valor $f(tx+(1-t)y)$ nunca supera la interpolación lineal $tf(x)+(1-t)f(y)$. Véase la \cref{fig:tema_1_funcion_convexa}.

  Esta segunda formulación no necesita derivadas ni siquiera que el dominio sea un intervalo de $\R$: solo necesita que tenga sentido hablar de segmentos, es decir, que el dominio sea un conjunto convexo de un espacio vectorial. Esto nos da la definición \emph{generalizada} de convexidad.
\end{informal}
\input{./fig/tema_1_funcion_convexa.tex}
\begin{definition}[Función convexa generalizada]
...
\begin{definition}[Sucesión acotada]
  Decimos que $(x^n)$ está acotada si existe $\epsilon >0$ tal que $\norm{x^n}<\epsilon$ para todo $n \in \N$ 
\end{definition}
Comprueba que toda sucessión convergente está acotada.
\begin{lemma}[Propiedades de las suceesiones]
Si $(x^n)\to x$ e $(y^n)\to y$, entonces 
\begin{itemize}
  \item $(\norm{x^n})\to \norm{x}$ 
  \item para una sucesión real $(\lambda^n)\to \lambda$, $(\lambda^n x^n+y^n)\to \lambda x+y$
  \item $(\ip{x^n, y^n})\to \ip{x,y}$ 
\end{itemize}
\end{lemma}
Demostramos primero la segunda propiedad.
\begin{proof}
    Sea $\epsilon >0$, vease que usando reiteradametne la desigualdad triangular tenemos que \[
      \norm{\lambda^n x^n+y^n-\lambda x-y}\leq \norm{\lambda^n x^n -\lambda x}+\norm{y^n-y},
    \] 
    y 
    \begin{align*}
    \norm{\lambda^n x^n -\lambda x}
    =\norm{\lambda^n x^n -\lambda x^n+\lambda x^n-\lambda x}\\
    = \norm{(\lambda^n -\lambda)x^n+(x-x^n)\lambda}\leq \abs{\lambda^n -\lambda}\norm{x^n}+\abs{\lambda} \norm{x^n-x};
    \end{align*}
    como $(x^n)$ converge, está acotada y existe $M>0$ tal que $\norm{x^n}<M$; finalmente basta tomar $n \in \N$ lo suficientemente grande para que $\norm{y^n-y}<0.5\epsilon$, $\abs{\lambda^n-\lambda}<0.25 M^{-1}\epsilon$ y $\norm{x^n-x}<0.25 \abs{\lambda}^{-1} \epsilon $. Hemos asumido que $\abs{\lambda} \neq 0$ al acotar $\norm{x^n-x}$, pero si se diese la igualdad entonces no haría falta acotar.
\end{proof}
Ahora la última.
\begin{proof}
    Asume que la norma está inducida por el producto interno. Sea $\epsilon >0$ 
    \begin{align*}
      ...
      \norm{x^k}\norm{y^k-y}+\norm{y}\norm{x^k-x},
    \end{align*}
    donde hemos usado \fullref{thm:cauchy-schwarz};
    como $(x^n)$ converge, está acotada y existe $M>0$ tal que $\norm{x^n}<M$; finalmente basta tomar $n_0 \in \N$ lo suficientemnate grande como para que $\norm{y^n-y}<0.5 M^{-1}\epsilon $ y $\norm{x^n-x}<0.5\norm{y}^{-1} \epsilon $.
Hemos asumido que $\norm{y} \neq 0$ al acotar $\norm{x^n-x}$, pero si se diese la igualdad entonces no haría falta acotar.
\end{proof}
\begin{lemma}
